...
function download() {
    let body_1 = "預覽:<br><textarea id='preview-area' readonly></textarea><br>檔名:<span class='file-name'></span><button class='btn btn-primary' onclick='rename_for_modal()' data-bs-dismiss='modal'>重新命名</button><br><button onclick='dl()' class='btn btn-primary'>下載</button>";
...
function LabAlert() {
    ...
    PopupAlert('實驗性功能:語音辨識', body, 'recognition.stop();', 'recognition.stop();textarea.val(textarea.val()+textBox.html()); txtHistory.record(textarea.val())',false);
